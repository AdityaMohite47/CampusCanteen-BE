...
from models import Message, User, SessionContext, Order
...
def add_message_to_mongo(message: Message):
    ...
    if not user_collection.find_one({"phone_number": message.phone_number}):
    ...
    try:
        ...
        if session:
            session_collection.update_one(
                ...
                {"$push": {"context_history": message.model_dump()}}
            )
        else:
...
def fetch_menu():
    ...
    try:
        ...
        raise PyMongoError(f"Failed to fetch menu: {str(e)}")
