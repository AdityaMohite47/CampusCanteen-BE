...
async def add_bot_message_to_mongo(msg : str , session_id : str , phone_number : str):
    BotMessage = Message(
...
async def process_message(message: Message) -> str:
    try:
        ...
        if response_data is not None and str(response_data).strip():
            ...
            try:
                await add_bot_message_to_mongo(str(response_data.content), message.session_id, message.phone_number)
                ...
                await add_bot_message_to_mongo(fallback_message, message.session_id, message.phone_number)
